...
(function() {
  ...
  exports.Writer = function() {
    var header, input;
    program.option('-f, --file [file]', 'File you want to start writing defaults to date+title.ext').option('-e, --extension [string]', 'ext you want to use.', 'md').option('-g, --guido [file]', 'json file with author. Default => ./guido.json', './guido.json');
    ...
    return input.question('Title: ', function(title) {
      ...
      return input.question('Date (' + defacto + ')', function(dateChosen) {
        ...
        return fs.readFile(program.guido, function(err, stuff) {
          var defactoauthor;
          if (!err) {
            defactoauthor = (JSON.parse(stuff)).author;
          }
          if (defactoauthor === void 0) {
            defactoauthor = '';
          }
          return input.question('Author: ' + defactoauthor, function(author) {
            ...
            if (program.file == null) {
              file = header.date + (header.title.toLowerCase().split(' ').join('')) + '.' + program.extension;
            }
            if (program.file != null) {
              file = program.file;
            }
